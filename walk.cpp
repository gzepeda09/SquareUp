...
int checkKeys(XEvent *e)
{
	...
	switch (key) {
		case XK_1:
			g.featureMode = 1;//Adding 1 and 0 for feature
			break;
		case XK_0:
			g.featureMode = 0;
			break;
...
void render(void)
{
	...
	r.left = 10;
	r.center = 0;
	if (g.featureMode == 1) {//Adding if else to switch between modes
        ggprint8b(&r, 16, c, "");
        ggprint8b(&r, 16, c, "");
        ggprint8b(&r, 16, c, "");
        ggprint8b(&r, 30, c, "          You are in *FEATURE MODE*");
        greenBoxes();
	} else {
